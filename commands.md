...
```env
PROJECT_ID="enterprise-rag-507805"
LOCATION="us-central1"
GCP_DOC_AI_LOCATION="us"
GCP_DOC_AI_PROCESSOR_ID="your doc ai processor id here"
GCP_RAW_BUCKET="rag-raw"
GCP_PROCESSED_BUCKET="rag-processed"
VPC_CONNECTOR="rag-vps"

QDRANT_API_KEY="Your qdrant api key"
QDRANT_CLUSTER_ENDPOINT="your qdrant cluster endpoint here"
...
gcloud run deploy rag-api \
  ...
  --set-env-vars "PROJECT_ID=your project id here" \
  --set-env-vars "LOCATION=your location here" \
  --set-env-vars "GCP_DOC_AI_PROCESSOR_ID=your doc ai processor id here" \
  --set-env-vars "GCP_RAW_BUCKET=your raw bucket here" \
  --set-env-vars "GCP_PROCESSED_BUCKET=your processed bucket here" \
  --set-env-vars "QDRANT_API_KEY=your qdrant api key here" \
  --set-env-vars "QDRANT_CLUSTER_ENDPOINT=your qdrant cluster endpoint here" \
  --set-env-vars "GROQ_API_KEY=your groq api key here" \
  --set-env-vars "LOGFIRE_TOKEN=your logfire token here" \
  --set-env-vars "LANGSMITH_TRACING=true" \
  --set-env-vars "LANGSMITH_PROJECT=entreprise_rag" \
  --set-env-vars "LANGSMITH_API_KEY=your langsmith api key here" \
...
The Ingestion Test: Upload a PDF to your `dmtxpress-rag-raw` bucket. Check the Cloud Run logs for the `ingestion` service—you should see it wake up automatically!
The Memory Test: Ask a question in the UI. Refresh the page. The agent should remember your name/context because it's now in Postgres.
The Speed Test: Ask the same question twice. The second response should be instant (< 100ms) because of the Redis Semantic Cache.
---
---